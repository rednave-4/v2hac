...
function openMissionPanel(mission) {
  ...
  document.getElementById('panel-title').textContent = mission.title;
  document.getElementById('panel-date').textContent = mission.date;
  document.getElementById('panel-desc').textContent = mission.desc;
  document.getElementById('panel-diff').textContent = '★'.repeat(mission.stars) + '  MISI';

  const st = document.getElementById('panel-status');
  const play = document.getElementById('panel-play');
  if (status === 'done') {
    st.textContent = '✓ Selesai — bisa dimainkan ulang';
    play.disabled = false;
    play.textContent = 'MAIN LAGI';
  } else if (status === 'available') {
    st.textContent = '● Tersedia — siap dimainkan';
    play.disabled = false;
    play.textContent = 'MULAI MISI';
  } else {
    st.textContent = '🔒 Terkunci — selesaikan misi sebelumnya';
    play.disabled = true;
...
const STAGE_MAPS = {
  ...
  rengasdengklok: [
    ...
    },
    {
      ...
      npcs: [], items: [],
      patrols: [
        {x:180,y:100,r:12,path:[[180,100],[180,400]],idx:0,speed:1.35,vision:70,facing:Math.PI/2},
        {x:320,y:200,r:12,path:[[250,200],[460,200]],idx:0,speed:1.2,vision:62,facing:0},
        {x:420,y:350,r:12,path:[[420,120],[420,400]],idx:0,speed:1.25,vision:60,facing:-Math.PI/2}
      ],
    ...
    if (m.alarmEnabled) {
      (m.patrols || []).forEach(pat => {
        ...
        if (inCone && !isHidden()) {
          game.beingSeen = true;
          // closer = faster alarm
          const prox = 1.2 + (1 - d / pat.vision) * 2.2;
          game.alarm += prox;
      ...
      if (!game.detected && !game.beingSeen && game.alarm > 0) {
        game.alarm = Math.max(0, game.alarm - 0.18);
      }
